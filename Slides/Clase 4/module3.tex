...
\begin{itemize}
    ...
    \item \textbf{Iteraci\'on de Pol\'itica:} Alterna entre evaluar $V^u$ (sistema lineal) y mejorar $u$ (\textit{greedy step}).
\end{itemize}
...
El paradigma de MDP es potente, pero enfrentamos barreras cr\'iticas al aplicarlo en la industria:

\begin{itemize}
    \item \textbf{Incertidumbre en los Par\'ametros:} 
    En un e-commerce (precificaci\'on), no conocemos la probabilidad real de que un cliente acepte un precio. 
    
    \item<2-> \textbf{Observabilidad Parcial:} 
    En la gesti\'on de relaciones con clientes (CRM), el ">estado emocional"< del cliente no se ve. Solo tenemos indicadores ruidosos (clics, quejas).
    
    \item<3-> \textbf{La Maldici\'on de la Dimensionalidad:} 
    Si gestionamos un inventario de 100 productos, \textquestiondown c\'omo ser\'a el espacio de estados? \textquestiondown Cu\'anto tiempo y espacio se necesitar\'a para calcular y almacenar $V^*$?
\end{itemize}

\begin{alertblock}{El quiebre del modelo}<4->
    Cuando las reglas ($P, r$) son desconocidas o el espacio es inabarcable, la Programaci\'on Din\'amica tradicional falla.
\end{alertblock}
\end{frame}

% --- SLIDE: HACIA EL REINFORCEMENT LEARNING ---
\begin{frame}{Desaf\'ios y sus Soluciones Modernas}
        \begin{block}{1. Informaci\'on Parcial}
        ...
        \begin{block}{2. Estados Ocultos}
            \small El estado real no es visible.
            \vspace{0.2cm}
            \textbf{\textcolor{red}{Soluci\'on: POMDPs}} \\
            Actuar en el espacio de probabilidades de estados.
        \end{block}

        \begin{block}{3. Complejidad Masiva}
            \small Millones de estados o acciones continuas.
            \vspace{0.2cm}
            \textbf{\textcolor{red}{Soluci\'on: Generalizaci\'on}} \\
...
\begin{itemize}
    \item Hasta ahora, dependíamos de $P$ y $r$. ¿Qué sucede cuando las reglas del mundo son desconocidas o demasiado complejas para modelar?
    
    \item<2-> \textbf{La Propuesta:} ">Aprender"< la política óptima directamente a partir de la \textbf{interacción} con el ambiente.

    \item<3-> \textbf{¿Con qué datos contamos?} En lugar de matrices, usamos experiencias:
    \begin{itemize}
        \item<4-> \textbf{Trayectorias:} Secuencias temporales de estados y acciones.
        \begin{itemize}
            \item<4-> Única (horizonte largo): $(x_0, a_0, r_0, x_1, a_1, r_1, \dots)$
            \item<5-> Múltiples (episodios): $\{(x_0^i, a_0^i, r_0^i, \dots, x_T^i)\}_{i=1}^M$
        \end{itemize}
        \item<6-> \textbf{Tuplas (Transiciones):} Conjuntos de experiencias aisladas $(x, a, r, y)$. 
    \end{itemize}
\end{itemize}
...
\begin{center}
\mode<beamer>{
\only<2,3>{\includegraphics[width=0.35\textwidth]{../Images/LearningCycle/RL2.jpg}} 
\only<4>{\includegraphics[width=0.35\textwidth]{../Images/LearningCycle/LearningCycle1.jpg}}  
\only<5>{\includegraphics[width=0.35\textwidth]{../Images/LearningCycle/LearningCycle2.jpg}} 
\only<6->{\includegraphics[width=0.35\textwidth]{../Images/LearningCycle/LearningCycle3.jpg}}} 
\end{center}
\end{frame}
...
Construir un \textbf{Modelo del Ambiente} suele ser la mejor opci\'on cuando el sistema posee una estructura que podemos explotar:

\begin{itemize}
    \item<2-> \textbf{Estructura y Ejemplos:}
    \begin{itemize}
        \item<3-> \textbf{Modelos Param\'etricos:} Ecuaciones de la f\'isica o econometr\'ia (ej. leyes de Newton, Black-Scholes). Pocos par\'ametros para estimar.
        \item<4-> \textbf{Digital Twins:} R\'eplicas digitales de alta fidelidad de activos f\'isicos (ej. una turbina e\'olica o una red el\'ectrica).
        \item<5-> \textbf{Simuladores Estoc\'asticos:} Modelos de eventos discretos o Monte Carlo para log\'istica y colas.
    \end{itemize}

    \item<6-> \textbf{Sistemas Cr\'iticos (Seguridad):} En medicina o energ\'ia, el costo de "fallar" es infinito. El modelo permite \textbf{testeo offline} y simulaciones de escenarios catastr\'oficos.
    
    \item<7-> \textbf{Eficiencia de Muestra:} La ventaja nace de la \textbf{estructura}. El modelo nos permite inferir estados nunca visitados generalizando informaci\'on de otros estados mediante la l\'ogica del sistema.
\end{itemize}
\end{frame}

% --- SLIDE: VENTAJAS DEL ENFOQUE MODEL-FREE ---
\begin{frame}{Aprendizaje por Refuerzo Directo (Model-Free)}
\small % Reducimos ligeramente el tamaño general para asegurar el ajuste
\begin{itemize}
    \item<1-> \textbf{Independencia del Modelo:} Aprendemos la política \textbf{mientras se actúa}, sin fijar la dinámica del ambiente.
    \begin{itemize}
        \item \textit{Dinámicas cambiantes:} Ideal para entornos no estacionarios (mercados, recomendación a usuarios).
    \end{itemize}

    \item<2-> \textbf{Robustez:} Evitamos el riesgo de \textbf{sesgo del modelo} (\textit{model bias}) al no imponer estructuras paramétricas rígidas.

    \item<3-> \textbf{Interacción con ">Cajas Negras"<:} Esencial cuando el modelo es un \textbf{simulador} complejo.
    \begin{itemize}
        \item No accedemos a $P$ o $r$; solo observamos transiciones.
        \item \textit{Ejemplos:} Videojuegos, procesos industriales, clima.
    \end{itemize}

    \item<4-> \textbf{Escalabilidad:} Permite el uso de \textit{Function Approximation} (redes neuronales) para manejar espacios de estados donde la Programación Dinámica tradicional colapsa.
\end{itemize}

\begin{exampleblock}{En resumen}<5->
...
\begin{alertblock}{La Confluencia: RL como Soluci\'on de Bellman}<2->
    Incluso con un modelo perfecto, la \textbf{maldici\'on de la dimensionalidad} puede impedir resolver Bellman de forma exacta. En esos casos, usamos algoritmos de RL no para aprender el mundo, sino como un m\'etodo de \textbf{programaci\'on din\'amica aproximada}.
\end{alertblock}
\end{frame}
...
\begin{alertblock}{Foco del Curso}<2->
    Nos concentraremos en \textbf{M\'etodos de Valor}, la base para entender c\'omo "resolver Bellman" a partir de datos.
\end{alertblock}
\end{frame}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Pros:} Natural para acciones continuas; \textbf{Contras:} Alta varianza en el aprendizaje.
    \end{itemize}
    
    \vspace{0.2cm}
    \item \textbf{Métodos Actor-Crítico:}
    \begin{itemize}
        \item El \textbf{Actor} propone la acción y el \textbf{Crítico} (basado en valor) evalúa si fue buena.
        \item Es el estándar en RL moderno (Deep RL) porque combina la estabilidad del valor con la flexibilidad de la política.
    \end{itemize}
\end{itemize}

\begin{exampleblock}{¿Por qué mencionarlos?}<2->
...
\begin{itemize}
    \item<1-> \textbf{TD-Learning (Temporal Difference):} Cómo aprender de la experiencia paso a paso sin esperar al final de la trayectoria.
    \item<2-> \textbf{Q-Learning:} El algoritmo ">off-policy"< por excelencia para aprender la acción óptima de forma directa.
    \item<3-> \textbf{Arquitecturas de Aproximación:} 
    \begin{itemize}
...
\begin{center}
\textbf{Idea Clave:} Usaremos RL como un \textit{solver} iterativo de la Ecuación de Bellman.
\end{center}
\end{frame}
...
\begin{itemize}
    \item<2-> \textbf{Visi\'on Te\'orica:} TD-learning es la versi\'on \textbf{estoc\'astica} de la Iteraci\'on de Valor. En promedio, la actualizaci\'on sigue la direcci\'on de la ecuaci\'on de Bellman.
...
\begin{itemize}
    ...
    \item<2-> \textbf{Relaci\'on con $V^*$:} Como $V^*(x) = \max_a Q^*(x,a)$, podemos reescribir Bellman:
    \[ Q^*(x,a) = r_a(x) + \alpha \sum_y P_a(x,y) \max_{a' \in A} Q^*(y,a') \]
    
    \item<3-> \textbf{La gran ventaja:} Con una aproximaci\'on $\hat Q$, la pol\'itica es \textbf{directa}:
...
\begin{itemize}
    ...
    \item \textbf{Selecci\'on de la Acci\'on:} 
    Resuelve el dilema de \textbf{Exploraci\'on vs. Explotaci\'on}. ¿Usamos lo que ya funciona o buscamos algo mejor?
...
\begin{alertblock}{Lecci\'on de Experiencia}
    La mayor\'ia de las fallas en proyectos de RL no est\'an en el algoritmo, sino en una mala calibraci\'on de estos pilares.
\end{alertblock}
\end{frame}
% --- SLIDE 1: EL DILEMA ---
\begin{frame}{Exploraci\'on: ¿Costo o Estrategia?}
Para que $\hat Q \to Q^*$, el agente debe \textbf{explorar suficientemente} el espacio de acciones. 

\begin{itemize}
    \item \textbf{Eficiencia:} El azar puro garantiza convergencia, pero es computacionalmente lento.
    
    \item \textbf{Online Learning (En Vivo):}
    Dilema econ\'omico: explorar es costoso (p\'erdida de recompensa real).

    \item \textbf{Offline Learning (Simulador):}
    Problema de cobertura: explorar asegura "iluminar" regiones clave de la tabla $Q$.
\end{itemize}
...
\begin{frame}{Desaf\'ios en la Pr\'actica (Temas Avanzados)}
...
\begin{center}
\textit{Las siguientes diapositivas presentan soluciones de ingeniería para estos retos.}
\end{center}
\end{frame}
...
\begin{center}
    \textbf{Pr\'oximamente:} La maldici\'on de la dimensionalidad y aproximaci\'on de funciones.
\end{center}
\end{frame}
...
$670302056198056 \approx 2 \times 10^{170}$ configuraciones v\'alidas
\end{itemize}
\end{itemize}
...
\begin{frame}{La “Maldici\'on de la Dimensionalidad”}
\begin{itemize}
    \item \textbf{Causa Ra\'iz:} El n\'umero de estados crece \textbf{exponencialmente} respecto al n\'umero de variables ($C^N$).
    \item \textbf{Inviabilidad Tabular:} En problemas reales (Go, Inventarios, Finanzas), el espacio de b\'usqueda es tan vasto que no se puede visitar, calcular ni almacenar.
\end{itemize}

\begin{exampleblock}{Ejemplo: Control de Inventarios (10 productos)}
    Si cada producto tiene 100 niveles de stock:
    \begin{itemize}
        \item Estados posibles: $100^{10} = 10^{20}$ (m\'as que granos de arena en la Tierra).
...
\begin{center}
    \textit{Si el espacio de estados es prohibitivo, la memoria debe ser reemplazada por la intuici\'on.}
\end{center}
\end{frame}
...
\begin{block}{Cambio de Paradigma}
    Ya no buscamos el valor exacto de cada casilla, sino los \textbf{pesos} $\theta$ que mejor describen la \textbf{estructura} del problema.
\end{block}}
...
\begin{tikzpicture}[
  ...
  output/.style={draw, rectangle, minimum width=2.5cm, minimum height=0.9cm, fill=blue!10, thick},
...
\node[output, right=of f2] (value) {Valor $\hat{V}(x, \theta)$};
...
\begin{column}{0.5\textwidth}
\begin{center}
\visible<2->{\includegraphics[width=0.9\textwidth]{../Images/Figures/StateAggregation.png}}
\end{center}
\end{column}
\end{columns}
\end{frame}
...
\begin{center}
\visible<2->{\includegraphics[width=0.7\textwidth]{../Images/Figures/NeuralNetwork.png}}
\end{center}
\end{frame}
...
\begin{frame}{RL con Aproximaci\'on: El Residuo de Bellman}
\begin{columns}[T]
...
Buscamos los pesos $\theta$ que minimizan la discrepancia recursiva del sistema:
\small
...
\begin{itemize}
    \item<2-> \textbf{La Soluci\'on: Muestreo de Monte Carlo.}
    Sustituimos el valor esperado por muestras reales de la experiencia:
    \begin{small}
    ...
    \end{small}
    \item<3-> \textbf{Iteraci\'on:} Ajustamos $\theta$ mediante descenso de gradiente para que nuestra predicci\'on actual se acerque al target calculado con la recompensa observada.
\end{itemize}
...
Para hallar el valor de la opci\'on, debemos asumir que el inversor actuar\'a de forma racional para maximizar su beneficio.

\begin{itemize}
    \item \textbf{Dependencia Estrat\'egica:} El valor de la opci\'on hoy es el resultado de la mejor estrategia posible de ejercicio en el futuro.
    \item \textbf{El Dilema Recurrente:} En cada instante $t$, el valor de la opci\'on es:
...
\begin{itemize}
    ...
    \item<3-> \textbf{Conclusi\'on:} La valuaci\'on es, en esencia, un problema de \textbf{parada \'optima} (\textit{optimal stopping}).
\end{itemize}
\end{frame}
...
\begin{block}{Importancia del Resultado (Black-Scholes-Merton)}
    ...
    \begin{itemize}
        ...
        \item El riesgo se "cancela" mediante la posibilidad de replicar la opci\'on operando el activo y bonos.
    \end{itemize}
\end{block}}
\end{frame}
...
\begin{itemize}
    \item \textbf{Estado ($s_t$):} El precio del activo subyacente en el periodo $t$.
    \item \textbf{Acci\'on ($a_t$):} 
        \begin{itemize}
            \item $a=0$: \textbf{Continuar} (mantener la opci\'on abierta).
            \item $a=1$: \textbf{Ejercer} (cobrar payoff y terminar episodio).
    ...
    \item \textbf{Recompensa ($R$):}
        \begin{itemize}
            ...
            \item $R(s, a=1) = \max(K - s, 0)$
        \end{itemize}
    \item \textbf{Factor de Descuento:} $\alpha = e^{-r_f \Delta t}$.
    \item \textbf{Transiciones ($P$):} Si $a=0$, el estado evoluciona seg\'un la din\'amica de riesgo neutro:
    \[ s_{t+1} = s_t e^{(r_f - \frac{\sigma^2}{2})\Delta t + \sigma \sqrt{\Delta t} \epsilon_t} \]
\end{itemize}

\begin{center}
    \small \textit{Este marco nos permite usar RL para encontrar la pol\'itica de parada \'optima y precificar la opci\'on.}
\end{center}
\end{frame}
...
La valuaci\'on de opciones americanas pertenece a la clase de MDPs conocidos como \textbf{problemas de parada \'optima}. El objetivo es aprender una política que decida el momento exacto $\tau$ para detener el proceso y capturar el valor.

\begin{itemize}
    \item \textbf{M\'ultiples Enfoques de RL:} 
    \begin{itemize}
        \item \textbf{Métodos de Valor (Value-based):} Aproximar $Q_t^*(s,a)$ mediante arquitecturas lineales o redes neuronales (DQN).
        \item \textbf{Programaci\'on Din\'amica Aproximada (ADP):} Resolver Bellman hacia atrás usando regresión para estimar el valor de continuación.
        \item \textbf{Policy Search:} Optimizar directamente los parámetros de una frontera de ejercicio (ej. buscar el umbral óptimo).
    \end{itemize}
    
    \item<2-> \textbf{Desafío del Estado Continuo:} Dado que el precio $s_t$ es continuo, no podemos usar tablas. Necesitamos una arquitectura de aproximaci\'on. Hoy vamos a utilizar una arquitectura lineal:
...
\begin{itemize}
    \item \textbf{Objetivo:} Aproximar la función de valor óptima $Q^*(s,a)$ cuando el espacio de estados es continuo.
    \item \textbf{Mecánica:} En lugar de una tabla, usamos un aproximador de funciones (en nuestro caso, regresión lineal): 
    \[ Q(s, a,; \theta) \approx \Phi(s, a)^T \theta \]
    \item \textbf{Iteración:} Se actualizan los parámetros $\theta$ aplicando el operador de Bellman sobre un conjunto de datos (trayectorias simuladas).
     \item Para valuaci\'on de opciones, es suficiente estimar $Q(s, 0; \theta) \approx \Phi(s) \theta$, el valor de esperar($a=0$).
\end{itemize}
\end{frame}
...
\begin{enumerate}
    \item **Stopping Time ($\tau^j$):** Primer $t$ donde ejercer es mejor que el valor de continuación predicho:
    \[ \tau^j = \min \{ t : K - s_t^j > \Phi(s_t^j, \theta_t) \} \]
    \item **Precio:** $\hat{V}_0 = \frac{1}{M} \sum_{j=1}^M \alpha^{\tau^j} \text{Payoff}(s_{\tau^j}^j)$
\end{enumerate}
\end{frame}
